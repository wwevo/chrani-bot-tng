...
logger = get_logger("dom_management.select_all")
...
def main_function(module, event_data, dispatchers_steamid):
    ...
    if all([
        ...
        if action in [  # only proceed with known commands
            "select_all_dom_elements",
            "deselect_all_dom_elements"
        ]:
            # Get all elements in the dataset
            all_elements = module.dom.data.get(target_module, {}).get("elements", {}).get(dom_element_origin, {})

            if not all_elements:
                event_data[1]["fail_reason"].append("no elements found")
                module.callback_fail(callback_fail, module, event_data, dispatchers_steamid)
                return

            # Build upsert structure for all elements
            elements_to_upsert = {}

            for element_owner, element_dict in all_elements.items():
                general_root = [target_module, "elements", dom_element_origin, element_owner]
                ...
                elements_to_upsert[element_owner] = nested_data

            # Single upsert for all elements
            if elements_to_upsert:
                module.dom.data.upsert({
                    target_module: {
                        "elements": {
                            dom_element_origin: elements_to_upsert
                        }
                    }
                ...
                event_data[1]["fail_reason"].append("no elements to update")
...
action_meta = {
    "description": "handles selecting or deselecting all elements in a dataset for further actions",
